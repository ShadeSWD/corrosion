...
(function () {
  ...
  function run() {
    ...
    out('o_q1', 'q₁ = ', `(${f(dvUm, 0)} + ${f(dft, 0)})/(10·${f(sv, 0)})·100/(100 − ${f(Z, 0)})`,
      f(q1, 3) + ' л/м²',
    ...
    out('o_n', 'n = ', `⌈${f(dfts, 0)}/${f(dft, 0)}⌉`,
      n + ' ' + plural(n, 'слой', 'слоя', 'слоёв'));
    const Q = S * (q1 + (n - 1) * q2);
    out('o_Q', 'Q = ', `${f(S, 0)}·[${f(q1, 3)} + ${n - 1}·${f(q2, 3)}]`,
      f(Q, 0) + ' л');
    const solp = num('sol');
    const Qs = Q * solp / 100;
    out('o_sol', 'Q_р = ', `${f(Q, 0)}·${f(solp, 0)}/100`, f(Qs, 0) + ' л');
    out('o_dfts', 'ΣDFT_факт = ', `${n}·${f(dft, 0)}`, f(n * dft, 0) + ' мкм',
      n * dft > dfts ? 'Фактическая толщина превышает требуемую: целое число слоёв редко ложится ровно.' : '');

    const nm = C.nMeasure(S);
    out('o_meas', 'N_замеров = ', `по площади ${f(S, 0)} м²`,
      nm + ' ' + plural(nm, 'замер', 'замера', 'замеров'),
  ...
  function draw(n, dft, dvUm, sv, Z, q1) {
    ...
    if (dvUm > 0) {
      const h = dvUm * k;
      y -= h;
      layers.push({ y, h, fill: '#e8d9c2', stroke: '#b45309', t: 'мёртвый объём ' + f(dvUm, 0) + ' мкм' });
    }
    for (let i = 0; i < n; i++) {
      const h = dft * k;
      y -= h;
      layers.push({ y, h, fill: i % 2 ? '#dcebe1' : '#cfe0d6', stroke: '#2f6f4f',
        t: `слой ${i + 1} — ${f(dft, 0)} мкм` });
    }
    ...
    const bars = parts.map((p) => {
      ...
      return `<rect x="${bx}" y="${by.toFixed(1)}" width="${bw}" height="${Math.max(h, 0).toFixed(1)}"
        fill="${p.c}" opacity=".8"/>` +
        (h > 13 ? `<text x="${bx + bw + 10}" y="${(by + h / 2 + 4).toFixed(1)}" class="cap">${p.t}: ${f(p.v, 3)} л/м², ${f(p.v / sum * 100, 0)} %</text>` : '');
    }).join('');

    el('board').innerHTML = `
      <text x="20" y="24" class="cap b">схема системы в масштабе толщин</text>
      <text x="20" y="42" class="cap gray">всего ${n} ${plural(n, 'слой', 'слоя', 'слоёв')} по ${f(dft, 0)} мкм = ${f(n * dft, 0)} мкм</text>
      ...
      <text x="326" y="42" class="cap gray">итого ${f(q1, 3)} л/м² при s.v. ${f(sv, 0)} % и потерях ${f(Z, 0)} %</text>
      ${bars}
