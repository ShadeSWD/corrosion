...
(function () {
  ...
  function run() {
    ...
    C.out('o_S', 'S = ', `1,025·${f(L, 0)}·(${f(Cb, 2)}·${f(B, 1)} + 1,7·${f(d, 1)})`,
      f(S, 0) + ' м²');

    const nu = C.nuSea(tw);
    const Re = v * L / nu;
    C.out('o_Re', 'Re = ', `${f(v, 3)}·${f(L, 0)}/${f(nu * 1e6, 4)}·10⁻⁶`,
      f(Re / 1e9, 3) + '·10⁹',
      `Скорость ${f(Vkn, 1)} уз = ${f(v, 3)} м/с; вязкость морской воды при ${f(tw, 0)} °C равна ${f(nu * 1e6, 4)}·10⁻⁶ м²/с.`);

    const CF = C.cf57(Re);
    C.out('o_CF', 'C_F = ', `0,075/(lg(${f(Re / 1e9, 3)}·10⁹) − 2)²`,
      f(CF * 1000, 4) + '·10⁻³');
    ...
    C.out('o_ksT', 'k_s(T) = ', `${f(ks0, 0)} + ${f(dks, 0)}·${f(Ty, 2)}`,
      f(ksT, 0) + ' мкм');
    ...
    C.out('o_delta', 'δ = ', `(${f(sT.PB, 0)} − ${f(s0.PB, 0)})/${f(s0.PB, 0)}·100`,
      f(delta, 2) + ' %');
    ...
    C.out('o_th', 't_ход = ', `${f(Tm, 0)}·730·${f(num('run'), 0)}/100`, f(th, 0) + ' ч');
    const sfoc = num('sfoc');
    const G0 = sfoc * s0.PB * th / 1e6;           // т
    C.out('o_G0', 'G₀ = ', `${f(sfoc, 0)}·${f(s0.PB, 0)}·${f(th, 0)}/10⁶`, f(G0, 0) + ' т');
    const G = sfoc * PBavg * th / 1e6;
    C.out('o_G', 'G = ', `${f(sfoc, 0)}·${f(PBavg, 0)}·${f(th, 0)}/10⁶`, f(G, 0) + ' т',
      `Средняя за период потребная мощность P̄_B = ${f(PBavg, 0)} кВт против ${f(s0.PB, 0)} кВт сразу после докования.`);
    const dG = G - G0;
    const price = num('price');
    C.out('o_dG', 'ΔG = ', `${f(G, 0)} − ${f(G0, 0)}`,
  ...
  function draw(track, PB0, Ty, ks0, ksT, delta) {
    ...
    el('board').innerHTML = `
      ...
      <text x="${x0 - 8}" y="${sy1(ks0) + 4}" text-anchor="end" class="cap gray">${C.f(ks0, 0)}</text>
      <text x="${x1 - 2}" y="${(sy1(ksT) - 8).toFixed(1)}" text-anchor="end" class="cap red">${C.f(ksT, 0)} мкм</text>
      ...
      <text x="${x1 - 2}" y="${(sy2(delta) - 8).toFixed(1)}" text-anchor="end" class="cap green">${C.f(delta, 1)} %</text>
